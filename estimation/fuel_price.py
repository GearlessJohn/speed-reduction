...
ifo380 = (pd.read_excel("./data/EU IFO380 Future Price.xlsx").dropna(subset=["PRIOR"])[["MONTH", "PRIOR"]].dropna())
...
ifo380_prices = (ifo380_year["PRIOR"] * price_ww_current["ifo380"] / price_eu_current["ifo380"]).to_numpy()

vlsfo = (pd.read_excel("./data/EU VLSFO Future Price.xlsx").dropna(subset=["PRIOR"])[["MONTH", "PRIOR"]].dropna())
...
vlsfo_prices = (vlsfo_year["PRIOR"] * price_ww_current["vlsfo"] / price_eu_current["vlsfo"]).to_numpy()

mgo = (pd.read_excel("./data/EU MGO Future Price.xlsx").dropna(subset=["PRIOR"])[["MONTH", "PRIOR"]].dropna())
mgo["MONTH"] = mgo["MONTH"].astype("datetime64[ns]")
mgo["PRIOR"] = mgo["PRIOR"].astype("float64")
mgo_year = mgo.groupby(mgo.MONTH.dt.year).mean()
mgo_prices = (mgo_year["PRIOR"] * price_ww_current["mgo"] / price_eu_current["mgo"]).to_numpy()

# 1 metric ton LNG = 52.0 million British thermal unists (MMBtu)
lng = (pd.read_excel("./data/EU LNG Future Price.xlsx").dropna(subset=["PRIOR"])[["MONTH", "PRIOR"]].dropna())
...
lng_prices = (lng_year["PRIOR"] * price_ww_current["lng"] / price_eu_current["lng"]).to_numpy()
