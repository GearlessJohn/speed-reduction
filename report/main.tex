...
\subsubsection*{Construction of new vessel}
...
\begin{equation}
	\label{eq:AC}
	AC(v) = VT(v)  \cdot v \cdot Capacity \cdot N
\end{equation}
...
\begin{equation}
	\label{eq:gap}
	GAP_{i} = max(1 - \frac{AC(v_i) \cdot N(i)}{AC(v_0) \cdot N_{ini}}, 0)
\end{equation}
where N(i) is the number of this kind of vessel in year i and $N_{ini}$ is its initial number in 2021.\\
...
\subsubsection*{Volatile freight rates}
...
We thus assume that the elasticity of freight rates with respect to supply is also 1.93.
...
\begin{equation}
	\label{eq:fr_mf}
	FR(i) = FR_{ini} \cdot(2.93 - 1.93 \cdot \frac{\sum_{j \in Container} AC_j(v_i) \cdot N_j(i)}{\sum_{j \in Container} AC_j(v_0) \cdot N_{j,ini}})
\end{equation}
...
\textcolor{red}{
	...
	\begin{enumerate}
		...
		\item If the conclusion in 1 holds, then set tolerance=0, and when the algorithm converges, it must converge to a fixed point. But does the algorithm necessarily converge?
	\end{enumerate}}
